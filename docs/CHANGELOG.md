...
- [新功能] 新增 `LLM_EXTRA_HEADERS` 全局配置，用于用户显式为 OpenAI 兼容中转站注入请求头；未配置时不改变官方 API 请求，渠道级 `LLM_{NAME}_EXTRA_HEADERS` 可覆盖同名字段。
- [修复] GitHub Actions 每日分析改为按计划 cron 与美东夏令/冬令状态识别 09:30-10:00 开盘后触发：夏令时对应北京时间 21:30-22:00，冬令时对应 22:30-23:00，避免 runner 排队延迟到窗口外时误跳过原本 09:30 的执行。
...
- [修复] 美股/港股实时行情链路改为支持与 A 股一致的补字段合并逻辑；当 Twelve Data 等首个成功源只返回基础价格字段时，后续 API 源仍可补充 `volume_ratio`、`turnover_rate`、估值与振幅等缺失指标，避免首个成功即提前返回导致量比等增强字段丢失。
...
- [修复] GitHub Actions `daily_analysis.yml` 补齐 `LITELLM_FALLBACK_MODELS` 注入，仓库 Secrets / Variables 中配置的统一 fallback 模型链在云端运行时可被实际读取与生效。
