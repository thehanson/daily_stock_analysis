...
默认配置已改为：**按美东 09:30-10:00 开盘后窗口自动执行**，对应北京时间通常为**夏令时 21:30-22:00、冬令时 22:30-23:00**。GitHub Actions 可能排队延迟启动，工作流会按计划 cron 与美东夏令/冬令状态判定，避免原本 09:30 的任务因实际启动较晚而误跳过。

修改时间：编辑 `.github/workflows/daily_analysis.yml` 中的 cron 表达式：

```yaml
schedule:
  - cron: '30 13 * * 1-5'  # 夏令时候选：北京时间 21:30-22:00
  - cron: '30 14 * * 1-5'  # 冬令时候选：北京时间 22:30-23:00
```

常用 cron 示例：
| 表达式 | 说明 |
|--------|------|
| `'30 13 * * 1-5'` + `'30 14 * * 1-5'` | 美东 09:30-10:00 开盘后窗口（夏令时对应北京时间 21:30-22:00，冬令时对应 22:30-23:00；工作流按计划 cron 与美东夏令/冬令状态判断） |
