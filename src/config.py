...
class Config:
    ...
    # Discord 机器人状态
    discord_bot_status: str = "多市场智能分析 | /help"
    ...
    def _load_from_env(cls) -> 'Config':
        ...
        return cls(
            ...
            llm_min_interval=parse_env_float(os.getenv('LLM_MIN_INTERVAL'), 1.0, field_name='LLM_MIN_INTERVAL', minimum=0.0),
    ...
    def _parse_market_review_region(cls, value: str) -> str:
        ...
        if v in ('cn', 'hk', 'us', 'both', 'all'):
            return v
        logging.getLogger(__name__).warning(
            f"MARKET_REVIEW_REGION 配置值 '{value}' 无效，已回退为默认值 'cn'（合法值：cn / hk / us / both / all）"
        )
        return 'cn'
    ...
    def get_db_url(self) -> str:
        ...
        优先使用外部 DATABASE_URL；未配置时回退到本地 SQLite，
        并自动创建 SQLite 数据库目录（如果不存在）。

        为兼容常见云平台返回的连接串，若使用 `postgres://`
        前缀则自动规范化为 SQLAlchemy 更稳定的 `postgresql://`。
        """
        external_db_url = (self.database_url or "").strip()
        if external_db_url:
            if external_db_url.startswith("postgres://"):
                external_db_url = "postgresql://" + external_db_url[len("postgres://"):]
