...
function normalizeChannelEnvSuffix(name: string): string {
...
function parseChannelsFromItems(items: Array<{ key: string; value: string }>): ChannelConfig[] {
  ...
  return channelNames.map((name) => {
    const upperName = normalizeChannelEnvSuffix(name);
...
function channelsToUpdateItems(
  ...
  const activeNames = channels.map((channel) => normalizeChannelEnvSuffix(channel.name));
  ...
  for (const channel of channels) {
    const prefix = `LLM_${normalizeChannelEnvSuffix(channel.name)}`;
  ...
  for (const oldName of previousChannelNames) {
    const upperName = normalizeChannelEnvSuffix(oldName);
...
export const LLMChannelEditor: React.FC<LLMChannelEditorProps> = ({
  ...
  const handleSave = async () => {
    ...
    if (hasEmptyName) {
      setSaveMessage({ type: 'local-error', text: '渠道名称不能为空，且只能包含字母、数字、下划线或连字符。' });
      return;
    }
