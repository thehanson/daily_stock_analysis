...
class TushareFetcher(BaseFetcher):
    ...
    def get_chip_distribution(self, stock_code: str) -> Optional[ChipDistribution]:
        """
        获取筹码分布数据（使用 cyq_perf 接口，平均成本使用 weight_avg）
        """
        ...
        try:
            ...
            trade_dates = self._get_trade_dates(current_date)
            if not trade_dates:
                print("[DEBUG] 无法获取交易日历")
                return None
            
            # 修正的逻辑：19:00之后优先使用当天数据（如果当天是交易日），否则使用最近交易日
            if current_hour >= 19:
                if current_date in trade_dates:
                    trade_date = current_date
                    print(f"[DEBUG] 当前时间 {current_hour} 点 >=19，且当天是交易日，使用当天数据: {trade_date}")
                else:
                    # 当天非交易日，取最近交易日
                    trade_date = trade_dates[0]
                    print(f"[DEBUG] 当前时间 {current_hour} 点 >=19，但当天非交易日，使用最近交易日: {trade_date}")
            else:
                # 19点前，使用前一交易日（避免数据未更新）
                # 取交易日列表中第二个（如果第一个是当天且当天为交易日，则取第二个；否则取第一个）
                if current_date in trade_dates and trade_dates[0] == current_date:
                    # 当天是交易日，但时间早于19点，取前一个交易日
                    trade_date = trade_dates[1] if len(trade_dates) > 1 else trade_dates[0]
                else:
                    trade_date = trade_dates[0]
                print(f"[DEBUG] 当前时间 {current_hour} 点 <19，使用前一交易日或最近交易日: {trade_date}")
            ...
            # 4. 使用 start_date 和 end_date 调用 cyq_perf，获取 weight_avg 作为平均成本
            ...
            df = self._call_api_with_rate_limit(
                ...
                end_date=trade_date,
                fields='ts_code,trade_date,winner_rate,cost_50pct,cost_5pct,cost_15pct,cost_85pct,cost_95pct,weight_avg,his_low,his_high'
            )
            ...
            cost_50pct_raw = row.get('cost_50pct', 0.0)     # 50分位成本（仅显示）
            cost_5pct_raw = row.get('cost_5pct', 0.0)
            cost_95pct_raw = row.get('cost_95pct', 0.0)
            cost_15pct_raw = row.get('cost_15pct', 0.0)
            cost_85pct_raw = row.get('cost_85pct', 0.0)
            weight_avg_raw = row.get('weight_avg', 0.0)     # 加权平均成本（作为平均成本）
            his_low_raw = row.get('his_low', 0.0)
            his_high_raw = row.get('his_high', 0.0)
            
            print(f"[DEBUG] 原始数据: winner_rate={winner_rate_raw}, cost_50pct={cost_50pct_raw}, "
                  f"cost_5pct={cost_5pct_raw}, cost_95pct={cost_95pct_raw}, "
                  f"cost_15pct={cost_15pct_raw}, cost_85pct={cost_85pct_raw}, "
                  f"weight_avg={weight_avg_raw}, his_low={his_low_raw}, his_high={his_high_raw}")
            
            # 6. 转换格式
            # 获利比例：百分比转小数
            profit_ratio = float(winner_rate_raw) / 100.0 if winner_rate_raw else 0.0
            # 平均成本：使用 weight_avg（加权平均成本）
            avg_cost = float(weight_avg_raw) if weight_avg_raw else 0.0
            # 如果 weight_avg 无效，尝试回退到 cost_50pct
            if avg_cost == 0.0 and cost_50pct_raw:
                avg_cost = float(cost_50pct_raw)
                print(f"[DEBUG] weight_avg 为0，回退使用 cost_50pct={avg_cost}")
            
            # 90%集中度：公式 (高-低)/(高+低)*100，然后转为小数
            if cost_5pct_raw and cost_95pct_raw and (cost_5pct_raw + cost_95pct_raw) != 0:
                concentration_90_pct = (cost_95pct_raw - cost_5pct_raw) / (cost_95pct_raw + cost_5pct_raw) * 100
                concentration_90 = concentration_90_pct / 100.0
            else:
                concentration_90 = 0.0
            # 70%集中度
            if cost_15pct_raw and cost_85pct_raw and (cost_15pct_raw + cost_85pct_raw) != 0:
                concentration_70_pct = (cost_85pct_raw - cost_15pct_raw) / (cost_85pct_raw + cost_15pct_raw) * 100
                concentration_70 = concentration_70_pct / 100.0
            else:
                concentration_70 = 0.0
            # 成本区间
            cost_90_low = float(cost_5pct_raw) if cost_5pct_raw else 0.0
            cost_90_high = float(cost_95pct_raw) if cost_95pct_raw else 0.0
            cost_70_low = float(cost_15pct_raw) if cost_15pct_raw else 0.0
            cost_70_high = float(cost_85pct_raw) if cost_85pct_raw else 0.0

            # 7. 构造 ChipDistribution 对象
